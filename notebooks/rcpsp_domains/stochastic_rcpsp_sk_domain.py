...
from skdecide.hub.space.gym import (DiscreteSpace, BoxSpace, ListSpace, SetSpace)
...
class StochasticRCPSPSGSDomain(D):
    def __init__(self, problem: Optional[RCPSPModel] = None,
        ...
        self.stochastic_dur = np.array(
            [[max(self.problem.mode_details[t][1]["duration"]-2,
        ...
        self.initial_state = np.zeros(
            [self.nb_tasks, 3], dtype=np.int64
        )
    ...
    def return_state(self):
        if self.params_domain_encoding.return_scheduled_in_state and self.params_domain_encoding.return_times_in_state:
            return self.state
    ...
    def _state_step(self, action: D.T_event) -> TransitionOutcome[D.T_state, Value[D.T_value], D.T_predicate, D.T_info]:
        ...
        if self.state[action, 0]:
            ...
            return TransitionOutcome(self.return_state(),
                                     Value(cost=self.params_domain_encoding.dummy_cost_when_already_schedule),
            ...
            if self.params_domain_encoding.nb_max_task_inserted is None or len(tasks)-1 <= self.params_domain_encoding.nb_max_task_inserted:
                if self.params_domain_encoding.use_cpm_for_cost:
                    _, m_pre = run_partial_classic_cpm(partial_schedule={self.problem.tasks_list[t]:
                                                              (self.state[t, 1],
                                                               self.state[t, 2])
                ...
                for k in tasks:
                    self.insert_task(k)
                if self.params_domain_encoding.use_cpm_for_cost:
                    _, m = run_partial_classic_cpm(partial_schedule={self.problem.tasks_list[t]:
                                                              (self.state[t, 1],
                                                               self.state[t, 2])
                                                              for t in range(self.problem.n_jobs)
                                                              if self.state[t, 0] == 1},
                ...
                return TransitionOutcome(state=self.return_state(),
                                         value=Value(cost=self.cur_makespan-pre
                                                     if self.params_domain_encoding.use_additive_makespan_for_cost
                                                     else m[self.problem.sink_task]._LSD - m_pre[self.problem.sink_task]._LSD),
                                         termination=term,
                                         info=None)
            else:
                return TransitionOutcome(state=self.return_state(),
    ...
    def insert_task(self, k: int):
        ...
        sample_dur = int(np.random.random_integers(self.stochastic_dur[k][0],
                                                   self.stochastic_dur[k][1],
                                                   size=1)[0])
        if sample_dur == 0:
            next_date = min_date
        else:
            next_date = next((t for t in range(min_date, 2*self.problem.horizon)
                              if all(np.min(self.resource_availability[p, t:t+sample_dur]) >= res_consumption[p]
    ...
    def _get_observation_space_(self) -> Space[D.T_observation]:
        if self.params_domain_encoding.return_times_in_state and self.params_domain_encoding.return_scheduled_in_state:
            return BoxSpace(low=0,
