...
from skdecide.hub.space.gym import GymSpace, DiscreteSpace, BoxSpace, MultiDiscreteSpace, ListSpace
from skdecide.domains import DeterministicPlanningDomain, RLDomain
import discrete_optimization
from discrete_optimization.rcpsp.solver.cpm import CPM, CPMObject, run_partial_classic_cpm
...
class RCPSP_LS_Domain(D):
    def __init__(self, problem: RCPSPModel):
        ...
        self.current_permutation = np.array([i for i in range(self.problem.n_jobs_non_dummy)])[::-1]
        self.resource_availability = np.copy(self.initial_resource_availability)
        self.scheduled_tasks = set()
        self.cur_makespan = 0
        self.records = []
        self.cpm = CPM(problem=self.problem)
        self.cur_sol = RCPSPSolution(problem=self.problem,
                                     rcpsp_permutation=self.current_permutation)
        self.state = np.array([self.cur_sol.rcpsp_schedule[t]["start_time"] for t in self.problem.tasks_list])
        self.cur_depth = 0
        self.cur_makespan = self.cur_sol.rcpsp_schedule[self.problem.sink_task]["start_time"]
    ...
    def _state_step(self, action: D.T_event) -> TransitionOutcome[D.T_state, Value[D.T_value], D.T_predicate, D.T_info]:
        ...
        self.current_permutation[min_i0:max_i1] = self.current_permutation[min_i0:max_i1][::-1]
        #self.current_permutation[i0] = self.current_permutation[i1]
        #self.current_permutation[i1] = pre
        pre_m = self.cur_makespan
        self.cur_sol = RCPSPSolution(problem=self.problem,
                                    rcpsp_permutation=self.current_permutation)
        self.cur_makespan = self.cur_sol.rcpsp_schedule[self.problem.sink_task]["end_time"]
        self.cur_depth += 1
        ...
        return TransitionOutcome(self.state,
                                 ...
                                 termination=self.cur_depth >= 50,
                                 info=None)

    def _get_action_space_(self) -> Space[D.T_event]:
        return BoxSpace(low=0, high=self.problem.n_jobs_non_dummy-1, dtype=int, shape=(2, ))
